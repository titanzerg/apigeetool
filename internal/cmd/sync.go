...
func RunSync(cfg ApigeeConfig, args SyncArgs) error {
	...
	table, targetEndpointsTable, targetTable, productsTable, appsTable, appCredentialsTable, proxyFlowsTable := syncTableNames(args)
	...
	if err := syncProxyEndpointsIfRequested(ctx, pool, selection, table, targetEndpointsTable, proxyFlowsTable, endpoints); err != nil {
		return err
	}
